...
import com.fightyz.lifecycle.ProductConnectionListener;
...
import com.fightyz.lifecycle.observer.LifecycleSubscriberInfoIndex;
import com.fightyz.lifecycle.observer.activity.ActivityObserver;
import com.fightyz.lifecycle.observer.drone.DroneObserver2;
...
public class Lifecycle implements ProductConnectionListener {
    ...
    public void onProductConnected() {
        for (AbstractObserver2 observer : observerList) {
            observer.notifyEvent(new ProductEvent(IEvent.ON_PRODUCT_CONNECTED));
        }
    }
    ...
    public void onProductDisconnected() {
        for (AbstractObserver2 observer : observerList) {
            observer.notifyEvent(new ProductEvent(IEvent.ON_PRODUCT_DISCONNECTED));
        }
    }
}
